...
class FusedMoE(torch.nn.Module):
    ...
    def _load_experts_on_demand(self, topk_output: TopKOutput):
        ...
        # offloaded layers. Both acc_offload (sparse_copy) and
        # transfer_weight paths are graph-safe, so the H2D copy is
        # captured in the graph and replayed on each forward — each
        # layer loads its own expert weights from DRAM.
        # Previously this was skipped during graph capture, causing all
        # layers to read stale data from the shared buffer (last warmup
        # layer's weights).
